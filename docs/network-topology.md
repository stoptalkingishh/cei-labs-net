...
| 32000–32767 | TCP | `cei-labs-engine`'s orchestrator: trusted per-instance gateway ports for single-target SSH and range-attacker SSH/noVNC, plus bulk-spawned analyst/Kali workspaces (see `ORCHESTRATOR_SSH_PORT_RANGE_START/END` and `ANALYST_BASE_PORT` in `cei-labs-engine`'s `docker/.env.example`) |
